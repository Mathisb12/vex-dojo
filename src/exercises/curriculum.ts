      ...
      // ── Lesson 1: What is VEX ──────────────────────────────────────────────
      {
        ...
        learnCards: [
          ...
          },
          {
            ...
            body: 'In Houdini, you write VEX inside a **Geometry Wrangle** (Attribute Wrangle) node. Its **Run Over** setting decides what your code loops over — it isn\'t always points:\n\n- **Points** (the default) — once per point, with `@ptnum` (current index) and `@numpt` (total)\n- **Primitives** — once per primitive/face, with `@primnum` and `@numprim`\n- **Vertices** — once per vertex, with `@vtxnum` and `@numvtx`\n- **Detail** — only **once total**, for whole-geometry setup, no per-component index\n\nThis lesson sticks to **Points** mode — by far the most common — but keep the other modes in mind once you\'re in real Houdini.',
            codeExample: '// Run Over = Points (this lesson)\n// Point 0 → @ptnum is 0\n// Point 1 → @ptnum is 1\n// ...\n// Last point → @ptnum is @numpt - 1',
            keyPoints: [
              'Run Over controls what the wrangle loops over: Points, Primitives, Vertices, or Detail',
              'Points (default): @ptnum / @numpt',
              'Primitives: @primnum / @numprim — Vertices: @vtxnum / @numvtx',
              'Detail: runs only once, no per-component loop at all',
            ],
          },
        ],
        exercises: [
          ...
          },
          {
            kind: 'mcq',
            id: 'intro-5',
            title: 'You set a wrangle\'s Run Over to Primitives instead of Points. What changes?',
            explanation: 'Changing **Run Over** changes what the code loops over and which built-ins are available: Primitives gives you `@primnum`/`@numprim` instead of `@ptnum`/`@numpt`. The same VEX syntax works in every mode — only the loop target and the index/total variables change.',
            choices: [
              { text: 'The code now runs once per primitive, with @primnum / @numprim instead of @ptnum / @numpt', correct: true },
              { text: 'Nothing — Run Over only affects the viewport display', correct: false },
